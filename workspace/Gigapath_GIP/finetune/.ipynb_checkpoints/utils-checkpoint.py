...
def get_regression_losses():
    return {'mse':torch.nn.MSELoss(), 'mae':torch.nn.L1Loss(), 'huber':torch.nn.HuberLoss(delta=0.5), 'logcosh':LogCoshLoss()}
...
def get_records_array(record_len: int, n_classes, save_embed: bool = False) -> dict:
