...
from training import train, test, generate_heatmap
...
if __name__ == '__main__':
    ...
    if not args.run_inference and not args.generate_heatmap:
        for fold in range(args.folds):
            ...
            train_data, val_data, test_data = DatasetClass(dataset, args.root_path, args.task_config, slide_key=args.slide_key, label = args.label, \
                                                           dataset_name = args.train_dataset, folds = args.train_fold, use_clinical_features = args.clinical_features) \
                                            , DatasetClass(dataset, args.root_path, args.task_config, slide_key=args.slide_key, label = args.label, \
                                                           dataset_name = args.val_dataset, folds = args.val_fold, use_clinical_features = args.clinical_features, test_on_all = args.test_on_all) if len(args.val_dataset) > 0 else None \
                                            , DatasetClass(dataset, args.root_path, args.task_config, slide_key=args.slide_key, label = args.label, \
                                                           dataset_name = args.test_dataset, folds = args.test_fold, use_clinical_features = args.clinical_features, test_on_all = args.test_on_all) if len(args.test_dataset) > 0 else None
        ...
        heatmap_dataset = DatasetClass(dataset, args.root_path, args.task_config, slide_key=args.slide_key, label = args.label, \
                                 dataset_name = args.test_dataset, folds = args.test_fold, use_clinical_features = args.clinical_features, \
                                       test_on_all = args.test_on_all, get_single_slide = args.get_single_slide, window_size = args.window_size)
        if args.clinical_features:
