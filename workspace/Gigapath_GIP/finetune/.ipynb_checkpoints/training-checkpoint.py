...
from utils import (get_optimizer, get_loss_function, \
                  ...
                  get_regression_losses, LogCoshLoss)
...
def generate_heatmap(heatmap_loader, args):
    ...
    scores = torch.abs(torch.autograd.grad(outputs=scores, inputs=images, grad_outputs=torch.ones_like(scores))[0].squeeze()).norm(2, dim=1)
    ...
    print(heatmap.shape)
    print(heatmap.max())
    np.save(os.path.join(args.save_dir,f'{args.get_single_slide}.npy'), heatmap)
...
def train(dataloader, fold, args):
    ...
    for i in range(args.epochs):
        ...
        torch.save(model.state_dict(), os.path.join(args.save_dir, 'fold_' + str(fold), 'model_epoch_'+str(i)+ '_checkpoint.pt'))

    # load model for test
    if test_loader is not None:
        model.load_state_dict(torch.load(os.path.join(args.save_dir, 'fold_' + str(fold), "checkpoint.pt")), strict = False)
...
def train_one_epoch(train_loader, model, fp16_scaler, optimizer, loss_fn, epoch, args):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16 if args.fp16 else torch.float32):
            ...
            elif isinstance(loss_fn, torch.nn.MSELoss) or isinstance(loss_fn, LogCoshLoss) or isinstance(loss_fn, torch.nn.L1Loss) or isinstance(loss_fn, torch.nn.HuberLoss):
                label = label.squeeze(-1).float()
